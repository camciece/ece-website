...
content:
  ...
  tr: |
    ...
    Bu kadar mı yani? Evet… ve hayır.

    Yukarıdaki kod tabii ki aşırı sadeleştirilmiş bir versiyon ama beni ilk şaşırtan şey şu olmuştu: Modern LLM’lerin iskeleti, düşündüğümüz kadar devasa ve karmaşık kod bloklarından oluşmuyor. Hatta bazı açık kaynak modellerin “tam çalışan” referans implementasyonları birkaç yüz satır Python koduyla yazılabiliyor. <a href="https://magazine.sebastianraschka.com/" target="_blank" rel="noreferrer">Sebastian Raschka</a> mesela, PyTorch ile sıfırdan model kurulumlarını adım adım gösteren harika eğitimler ve repo’lar paylaşıyor. Şu an oldukça güçlü kabul edilen açık modellerden biri olan OLMo-3’ün temel mimarisi de, yanlış hatırlamıyorsam, sadece birkaç yüz satırlık bir kodla ayağa kaldırılabiliyor. Tabii burada küçük ama kritik bir fark var. Kod kısa olabilir, ama o kodun içinde kullanılan ağırlıklar (weights) devasa. Asıl “zeka” dediğimiz şey, bu mimarinin içine eğitim sırasında doldurulan milyarlarca sayının içinde yaşıyor. Yani motorun şeması basit, ama motoru besleyen yakıt aşırı yoğun. Bu noktada artık hattın ilk durağına rahatça girebiliriz: Bu metin, bu sisteme ilk girdiğinde başına ne geliyor?

    Tokenizer ile başlayalım.
---
